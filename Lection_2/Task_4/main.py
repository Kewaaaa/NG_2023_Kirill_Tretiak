...
if choise == 1:
    ...
    for books, booksInfo in library.items():
        print(books)
        # I used this construction all time, because according to the task, i can't use function
        booksName = booksInfo["Title"]
        # It shows all books in better view
...
if choise == 4:
    for i in range(1, len(library) + 1):
        print("-------------")
        print(library[f"Book_{i}"]["Title"],
              "-", library[f"Book_{i}"]["Pages"], "pages")
    print("-------------")
    findBook = input(
